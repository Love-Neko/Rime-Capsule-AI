...
- **中英文切换按键自定义**：默认左右 `Shift` 均可切换；更支持在设置面板自定义为仅 `左Shift`、仅 `右Shift`（避开打字误触）、单键 `Ctrl`（彻底屏蔽 Shift 误触）或彻底禁用单键，并支持自定义编码上屏动作
...
| 操作 / 功能 | 快捷键 | 说明 |
| :--- | :--- | :--- |
| **中英模式切换** | `Shift` 单键 (默认) | 支持在设置面板自由自定义为 `左右Shift` / `左Shift` / `右Shift` / `Ctrl` 单键 / 禁用防误触 |
